...
class ProjectApiTest extends AbstractTestApi
{
    ...
    {
        $this->mockRequestGet('/projects/1/settings', '{
              "data": {
                "projectId": 1,
        ...
        $projectSettings = $this->crowdin->project->getSettings(1);
        $this->assertInstanceOf(ProjectSetting::class, $projectSettings);
        $this->assertEquals(1, $projectSettings->getProjectId());

        $projectSettings->setAutoSubstitution(false);

        $this->mockRequestPath('/projects/1/settings', '{
              "data": {
                "projectId": 1,
                "translateDuplicates": 1,
                "isMtAllowed": true,
        ...
        $projectSettings = $this->crowdin->project->updateSettings($projectSettings);
        $this->assertInstanceOf(ProjectSetting::class, $projectSettings);
        $this->assertEquals(1, $projectSettings->getProjectId());
        $this->assertFalse($projectSettings->isAutoSubstitution());
    }
